...
im_size = 512
out_size = 256
data_path = '/home/ubuntu/projects/ZheDa-Harvard/DSAC/liverpng568/'
csvfile=open(data_path+'polygons_liver.csv', newline='')
reader = csv.reader(csvfile)
images = np.zeros([im_size,im_size,3,568])
masks = np.zeros([out_size,out_size,1,568])
GT = np.zeros([L,2,568])
for i in range(568):
    ...
    this_im  = scipy.misc.imread(data_path+'volume-'+str(i+1)+'.png')
    images[:,:,:,i] = np.float32(this_im)/255
    img_mask = scipy.misc.imread(data_path+'mask-' + str(i+1) + '.png')/255
...
with tf.device('/gpu:0'):
    ...
    grad_predA, grad_predB, grad_predK, grad_l2loss, x, y_ = CNN_B(im_size, out_size, L, batch_size=4,wd=0.01,layers=len(numfilt),numfilt=numfilt)
...
elif os.path.isdir(model_path+'results/polygons568.csv'):
    os.remove(model_path+'results/polygons568.csv')
...
def epoch(n,i,mode):
    ...
    init_u = out_size / 2 + 20 * np.cos(s)-30
    init_v = out_size / 2 + 20 * np.sin(s)+10
...
with tf.Session(config=tf_config) as sess:
    ...
    if do_train:
        ...
        polygons_csvfile = open(model_path + 'results/' 'polygons568.csv', 'a', newline='')
    ...
    for n in range(start_epoch,end_epoch):
        ...
        iou_train = 0
        iter_count = 0
        #tf.scalar_summary('iou_train',iou_train)
        #tf.scalar_summary('iou_test',iou_test)
        if do_train:
            for i in range(0,400,batch_size):
                ...
                iter_count += 1
                print('Train. Epoch ' + str(n) + '. Iter ' + str(iter_count) + '/' + str(400/batch_size) + ', IoU = %.2f' % (
                iou_train / iter_count))
            iou_train /= iter_count
            print('Train, iou = %.2f' % (iou_train/(400/batch_size)) , file=f)
            saver.save(sess,model_path+'model', global_step=n)
        iter_count = 0
        for i in range(400,568,batch_size):
            ...
            iter_count += 1
            print('Test. Epoch ' + str(n) + '. Iter ' + str(iter_count) + '/' + str(168/batch_size) + ', IoU = %.2f' % (
            iou_test / iter_count))
        iou_test /= iter_count
        print('Test, iou = %.2f' % (iou_test/(168/batch_size)) , file=f)
        #file_writer = tf.summaryFileWriter('models/liverpng568/logs',sess.graph)
        if not do_train:
